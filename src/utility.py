...
class checkpoint():
    def __init__(self, args):
        ...
        if not args.load:
            if not args.save:
                args.save = now
            self.dir = os.path.join('..', 'experiment', args.save)  # 如果没有给名字就以时间命名
        else:
    ...
    def add_log(self, log, sigma_log=None):
        ...
        if sigma_log is not None:
            self.sigma_log = torch.cat([self.sigma_log, sigma_log])
    ...
    def save_results(self, dataset, filename, save_list, scale):
        if self.args.save_results:
            ...
            for v, p in zip(save_list, postfix):
                normalized = v[0].mul(255 / self.args.rgb_range) # to 0-255
                tensor_cpu = normalized.byte().permute(1, 2, 0).cpu() # to int
                self.queue.put(('{}{}.png'.format(filename, p), tensor_cpu))

def quantize(img, rgb_range):
    # quantize to 0-255 first, then to int and finally to 0-1
