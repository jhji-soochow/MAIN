...
# # _worker_manager_loop --> _pin_memory_loop
...
#  the following code works for pytorch1.1 : pytorch1.1 merges the aboved called functions into _utils.
...
def _ms_loop(dataset, index_queue, data_queue, collate_fn, scale, seed, init_fn, worker_id):
    ...
    np.random.seed(worker_id) #numpy
    while True:
        r = index_queue.get()
        if r is None:
            break
        ...
        try:
            ...
            if len(scale) > 1 and dataset.train:
                ...
                idx_scale = np.random.randint(len(scale))
...
class _MSDataLoaderIter(_DataLoaderIter):
    def __init__(self, loader):
        ...
        if self.num_workers > 0:
            ...
            # self.worker_result_queue = multiprocessing.SimpleQueue() # pytorch 0.4.1
            self.worker_result_queue = multiprocessing.Queue() # pytorch 1.0
            ...
            if self.pin_memory or self.timeout > 0:
                ...
                self.pin_memory_thread = threading.Thread(
                    target=_utils.pin_memory._pin_memory_loop,   # for pytorch1.0 target=_pin_memory_loop,
